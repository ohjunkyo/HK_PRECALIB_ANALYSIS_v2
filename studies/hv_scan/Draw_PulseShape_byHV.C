// Usage:  root -l -b -q Draw_PulseShape_byHV.C
//
#include "TFile.h"
#include "TTree.h"
#include "TH2D.h"
...
static const std::vector<HVPoint> kPoints = {
    {"20260829", 111, "-200V"}, {"20260829", 211, "-150V"}, {"20260829", 11, "-100V"},
    {"20260827", 11,  "-50V"},  {"20260815", 11,  "Recom."}, {"20260827", 111, "+50V"},
    {"20260830", 11,  "+100V"}, {"20260828", 11,  "+150V"}, {"20260828", 111, "+200V"},
};
...
void DrawPulseGrid(int ch, const char* chLabel, TTree* avgTree,
    ...
    TCanvas* c = new TCanvas(cname, "pulse", 1600, 1400);
    c->Divide(3, 3);
    ...
    for (size_t i = 0; i < kPoints.size(); ++i) {
        ...
        // (B) average of each EVENT'S OWN crossing time, vs (A) the crossing
        // of the averaged waveform (tThrR below) -- these are NOT the same:
        // finding a threshold crossing is nonlinear, so averaging first (A)
        // lets each event's own TTS jitter smear the rising edge, shifting
        // where the AVERAGE curve crosses threshold earlier than the true
        // per-event average. Confirmed ~4ns apart on the B-field campaign
        // (2026-09-05, user: "Pulse Average의 시간 정보와 각 Pulse의 시간
        // 정보를 평균한 것의 차이가 없겠지?" -- there is one). (B) matches
        // how prod_ntp_v7.C's official diff/TTS is computed (per event, then
        // averaged/fit), so it is the one time-walk conclusions should use.
        double sumTfallAbs = 0, sumTfallAbs2 = 0;
        // Per-event (tFall, tFall-tTrig) pairs -- needed to filter (B) down
        // to real prompt-peak hits after the fact (dark/noise crossings
        // scattered across the 40-sample search window were inflating (B)'s
        // spread to ~4-5x the real TTS(sigma) and making the A-B gap itself
        // drift with HV instead of being one clean offset) -- 2026-09-05,
        // user: "이벤트가 있는 것만 확인해야하나 그러면".
        std::vector<double> hitTfall, hitDiffRel;
        ...
        if (fCache && !fCache->IsZombie()) {
            ...
            // meta size 6 now (was 4) -- older caches auto-invalidate below,
            // forcing a recompute that fills the new fields instead of
            // reading garbage/out-of-range.
            TVectorD* htf = (TVectorD*)fCache->Get("Waveform_hitTfall_" + key);
            TVectorD* hdr = (TVectorD*)fCache->Get("Waveform_hitDiffRel_" + key);
            if (hc && hdc && sv && meta && htf && hdr && sv->GetNoElements() == nSamples && meta->GetNoElements() >= 6
                    && htf->GetNoElements() == hdr->GetNoElements()) {
                ...
                sumTfallAbs = (*meta)[4]; sumTfallAbs2 = (*meta)[5];
                hitTfall.assign(htf->GetMatrixArray(), htf->GetMatrixArray() + htf->GetNoElements());
                hitDiffRel.assign(hdr->GetMatrixArray(), hdr->GetMatrixArray() + hdr->GetNoElements());
                cacheHit = true;
            }
        }
        ...
        if (!cacheHit) {
            ...
            for (long long e = 0; e < n; ++e) {
                ...
                if (tTrig >= 0) {
                    hDiff->Fill(tFall - tTrig);
                    hitTfall.push_back(tFall);
                    hitDiffRel.push_back(tFall - tTrig);
                }
                tN++;
                sumTfallAbs += tFall; sumTfallAbs2 += (double)tFall * tFall;
            ...
            if (fCache && !fCache->IsZombie()) {
                ...
                TVectorD meta(6);
                meta[0] = n; meta[1] = tN; meta[2] = phSum; meta[3] = phSum2;
                meta[4] = sumTfallAbs; meta[5] = sumTfallAbs2;
                meta.Write("Waveform_meta_" + key, TObject::kOverwrite);
                TVectorD htf(hitTfall.size()); for (size_t k = 0; k < hitTfall.size(); ++k) htf[k] = hitTfall[k];
                htf.Write("Waveform_hitTfall_" + key, TObject::kOverwrite);
                TVectorD hdr(hitDiffRel.size()); for (size_t k = 0; k < hitDiffRel.size(); ++k) hdr[k] = hitDiffRel[k];
                hdr.Write("Waveform_hitDiffRel_" + key, TObject::kOverwrite);
            }
        }
  ...
  //      h->GetXaxis()->SetTitleSize(0.045); 
    //    h->GetYaxis()->SetTitleSize(0.045); 
      //  h->GetYaxis()->SetLabelSize(0.045); //h->GetYaxis()->SetTitleFont(132);
        ...
        // (B), filtered to the prompt peak -- see the long comment where
        // hitTfall/hitDiffRel are filled above. Without this cut, (B) folded
        // in every threshold crossing across the whole 40-sample search
        // window (kHitLo..kHitHi), including dark/noise crossings far from
        // the real laser peak -- that gave (B) an RMS of ~6.5-7.5ns (4-5x
        // the true TTS sigma) and made the A-B gap itself drift with HV
        // (~0ns at -200V -> ~5.5ns at +200V) instead of the two methods
        // differing by one clean, HV-independent offset. Restricting to
        // |diff - diffPk| <= 8 samples -- the SAME window the TTS exGaus fit
        // above uses -- keeps only real prompt hits.
        double tThrB = std::nan(""), tThrBRms = std::nan("");
        long long nFiltB = 0;
        if (std::isfinite(diffPk)) {
            double sumF = 0, sumF2 = 0;
            for (size_t k = 0; k < hitTfall.size(); ++k) {
                if (std::abs(hitDiffRel[k] - diffPk) > 8.0) continue;
                sumF += hitTfall[k]; sumF2 += hitTfall[k] * hitTfall[k];
                nFiltB++;
            }
            if (nFiltB > 0) {
                tThrB = sumF / nFiltB;
                tThrBRms = nFiltB > 1 ? std::sqrt(std::max(0.0, sumF2 / nFiltB - tThrB * tThrB)) : 0.0;
            }
        }
        ...
        if (std::isfinite(tThrR))
            row(Form("Thr(A, wf-avg) @ %.1f ns", tThrR * NS_PER_SAMPLE), kRed);
        if (std::isfinite(tThrB))
            row(Form("Thr(B, evt-avg) @ %.1f ns", tThrB * NS_PER_SAMPLE), kSpring - 1);
        pt->Draw();

        printf("[%-6s] ch%d  <PH>=%6.2f+-%5.2f mV  mean-wf peak=%6.2f  rise=%5.1f  fall=%5.1f  "
                "FWHM=%5.1f  TTS_sigma=%5.2f ns  thrA@%7.1f ns  thrB@%7.1f+-%.2f ns (n_prompt=%lld)  (A-B=%+.2f ns)  "
                "fall90@%7.1f ns  fall10@%7.1f ns  hit=%5.1f%% (n=%lld/%lld)\n",
                p.label, ch, phMean, phRms, amp, rise, fall, fwhm, tts,
                tThrR * NS_PER_SAMPLE, tThrB * NS_PER_SAMPLE, tThrBRms * NS_PER_SAMPLE, nFiltB,
                (tThrR - tThrB) * NS_PER_SAMPLE,
                t90f * NS_PER_SAMPLE, t10f * NS_PER_SAMPLE, hitFrac, tN, n);
    ...
    TString outPath = outDir + Form("PulseShape_byHV_ch%d.pdf", ch);
...
void Draw_PulseShape_byHV() {
    TString outDir = "./Data/image/ScanReport/";
    gSystem->mkdir(outDir, kTRUE);
    TString ntuplePath = outDir + "PulseShape_MeanWaveform.root";
