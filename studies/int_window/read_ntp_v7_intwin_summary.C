// Usage: root -l -b -q 'read_ntp_v7_intwin_summary.C(20260831,11,100,"Rep1")'   // (dateTag, run, intWindowNs, repTag)
#include <fstream>
#include <vector>
#include <string>
#include <iostream>
...
void read_ntp_v7_intwin_summary(int dateTag, int run, int intWindowNs, const char* repTag = "Rep1",
                                 const char* csvPath = "/home/precalkor/ADC/ADC_test/Data/summary/intwin_qe_tts_gain.csv") {
    ...
    {
        // must match prod_ntp_v7_intwin.C's own output naming
        openPath = Form("/home/precalkor/ADC/ADC_test/Data/production/IntWindowStudy/"
                        "precal_prd_kor_run_%d_%03d_int%dns.root", dateTag, run, intWindowNs);
        if (gSystem->AccessPathName(openPath.c_str())) {
            std::cerr << "[ERROR] not found: " << openPath << " -- run prod_ntp_v7_intwin.C first." << std::endl;
            return;
        }
        TString baseName = gSystem->BaseName(openPath.c_str());
        baseName.ReplaceAll("prd", "result");
        std::string outDir = std::string(FinalResultPath) + "/IntWindowStudy";
        gSystem->mkdir(outDir.c_str(), kTRUE);
        savePath = outDir + "/" + baseName.Data();
    }
    ...
    for (size_t i = 0; i < activeChannels.size(); ++i) {
        ...
        // One CSV row per channel -- what Draw_IntWindow_Compare.C reads.
        {
            std::ofstream csvOut(csvPath, std::ios::app);
            csvOut << repTag << "," << run << "," << ch << "," << pmtSN.Data() << "," << axisLabel << ","
                   << Form("%.3f", tiltVal) << "," << Form("%.3f", rotVal) << "," << intWindowNs << ","
                   << Form("%.4f", qeDarkCorrB) << "," << Form("%.4f", qeAbsB) << ","
                   << Form("%.4f", ttsB * 2.0) << "," << Form("%.4f", signalMean) << ","
                   << Form("%.4f", chargeResolution) << "\n";
        }
    ...
    std::string imageDir = ImagePath + "IntWindowStudy/"; gSystem->mkdir(imageDir.c_str(), kTRUE); 
