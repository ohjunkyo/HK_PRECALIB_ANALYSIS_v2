// Usage: root -l -b -q 'prod_ntp_v7_intWindow.C(20260831,11,11,100)'   // (dateTag, startRun, endRun, intWindowNs)
//
#include <vector>
#include <string>
#include <iostream>
...
void ProdOneRun(int dateTag, int run, int intWindowNs) {
    ...
    {
        static const char* dirs[2] = {"/home/precalkor/ADC/ADC_test/Data/RAW/Laser",
                                       "/home/precalkor/external_HDD_1_4T/Data_Backup/RAW/Laser"};
        for (int d = 0; d < 2; ++d) {
            TString cand = Form("%s/precal_raw_kor_run_%d_%03d.root", dirs[d], dateTag, run);
            if (!gSystem->AccessPathName(cand)) { OpenPath = cand.Data(); break; }
        }
        if (OpenPath.empty()) { std::cerr << "[ERROR] RAW not found for " << dateTag << "_" << run << std::endl; return; }

        TString baseName = gSystem->BaseName(OpenPath.c_str());
        baseName.ReplaceAll("raw", "prd");
        baseName.ReplaceAll(".root", Form("_int%dns.root", intWindowNs));
        std::string outDir = std::string(ProcessedDataPath) + "/IntWindowStudy";
        gSystem->mkdir(outDir.c_str(), kTRUE);
        outName = outDir + "/" + baseName.Data();
    }
    ...
    int sigWidthSamples = intWindowNs / 2;   // charge integration width, 2 ns/sample
    int sigEnd   = (sigStart + sigWidthSamples > nSamples) ? nSamples : sigStart + sigWidthSamples;
...
void prod_ntp_v7_intWindow(int dateTag, int startRun, int endRun, int intWindowNs = 100) {
    for (int r = startRun; r <= endRun; ++r) ProdOneRun(dateTag, r, intWindowNs);
    std::cout << "[INFO] prod done: " << dateTag << " runs " << startRun << "-" << endRun
              << " @ " << intWindowNs << "ns" << std::endl;
}
