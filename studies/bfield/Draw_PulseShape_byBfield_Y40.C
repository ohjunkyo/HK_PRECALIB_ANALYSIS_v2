// Usage:  root -l -b -q Draw_PulseShape_byBfield_Y40.C
//
#include "TFile.h"
#include "TTree.h"
#include "TH2D.h"
...
// Y-axis, tilt=40deg -- the point where Rot1's Raw QE Dev[%] dipped hardest
// across the B-field campaign -- 2026-09-05, user: "저기 Y축 40도가 가장 많이
// 변하는 지점같은데, 저기도 비교해줄래?" (center-run version is
// Draw_PulseShape_byBfield.C).
static const std::vector<HVPoint> kPoints = {
    {"20260831", 42,  "Nominal"}, {"20260831", 142, "B=0 (Rot1 target)"},
    {"20260901", 142, "Bx=-70mG"}, {"20260901", 242, "Bx=-150mG"},
};
...
//        h->GetXaxis()->SetLabelSize(0.045); //h->GetXaxis()->SetTitleFont(132);
  //      h->GetXaxis()->SetTitleSize(0.045); 
    ...
    TString outPath = outDir + Form("PulseShape_byBfield_Y40_ch%d.pdf", ch);
...
void Draw_PulseShape_byBfield_Y40() {
    TString outDir = "./Data/image/ScanReport/";
    gSystem->mkdir(outDir, kTRUE);
    TString ntuplePath = outDir + "PulseShape_MeanWaveform_Bfield_Y40.root";
