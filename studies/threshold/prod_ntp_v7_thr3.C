...
// 3mV comparison build. Kept as the ONE shared constant, exactly as in
// prod_ntp_v7.C: it drives the amplitude cut, the diff/timing crossing AND the
// dark-count calculation together. A software re-filter of the 1.5mV NTPs
// cannot stand in for this -- verified 2026-09-02 on 20260815_011, where the
// re-filter reproduced Raw QE exactly but diverged from the Timing-cut stage
// onward (20260815_011: Rot1 timing 1.5633 vs 1.4767, +5.9%). Raw agreeing
// exactly confirms the amplitude cut itself is equivalent; the divergence
// enters with the timing gate, whose window is re-centred on each production's
// own Diff peak and whose crossing time is defined at that production's own
// threshold. Both differ between a 1.5mV and a 3.0mV production, so the timing
// stage cannot be reproduced by re-filtering amplitude alone.
...
void prod_ntp_v7_thr3(int run, const char* rawFilePath = "") {
    ...
    if (strlen(rawFilePath) > 0) {
        ...
        outName = "./Data/production_thr3_compare/" + std::string(baseName.Data());
    } else {
    ...
    // Pedestal Stability: mean pedestal per event-index bin, error bar = RMS in
    // that bin (SetErrorOption("s") above) -- shows baseline drift/noise over
    // the course of the run, replacing the Charge-vs-Time (QT) canvas above.
    // Also computed here as scalars (pedMeanOut/pedSigmaOut, mV) so they can be
    // saved as TParameters below and later plotted vs angle across a whole scan
    // the same way NoiseCountRate_ch/etc. already are.
