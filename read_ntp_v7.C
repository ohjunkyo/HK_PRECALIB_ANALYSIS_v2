...
void read_ntp_v7(int run, const char* processedFilePath = "") {
    ...
    if (strlen(processedFilePath) > 0) {
        ...
        savePath = std::string(FinalResultPath) + "/" + baseName.Data(); 
    } else { 
    ...
    for (size_t i = 0; i < activeChannels.size(); ++i) {
        ...
        outTrees[i]->Branch("fwhm_exG", &fwhmB, "fwhm_exG/D");
        outTrees[i]->Branch("fwhm_exG_err", &fwhmErrB, "fwhm_exG_err/D");
        outTrees[i]->Branch("sigma_exG", &sigmaB, "sigma_exG/D");
        outTrees[i]->Branch("sigma_exG_err", &sigmaErrB, "sigma_exG_err/D");
        ...
        if (ch == TriggerCh) {
            ...
            fwhmB=0; fwhmErrB=0; sigmaB=0; sigmaErrB=0;
