...
int main(int ac, char *av[]) {
    ...
    if (root) {
        ...
        tree->Branch("PostTrigger", &post_trigger, "PostTrigger/i");
        tree->Branch("ADC", adc_buffer, Form("ADC[8][%d]/s", recordlength));

        for (int i = 0; i < nchannels; i++) {
            int ch = activechannels[i];
            tree->Branch(Form("OffsetValue%d", ch), &offset_value2[ch], Form("OffsetValue%d/i", ch));
            tree->Branch(Form("TriggerValue%d", ch), &trigger_value2[ch], Form("TriggerValue%d/i", ch));
        }
    } else { 
